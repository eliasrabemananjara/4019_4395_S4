...
-- Préfixes autorisés
INSERT INTO
    prefixes_operateur (idOperateur, prefixe)
VALUES (1, '032'),
    (3, '033'),
    (2, '034'),
    (1, '037'),
...
WHERE
    toper.libelle IN ('Retrait', 'Transfert');

-- VUE : total des gains par operateur
CREATE VIEW vue_total_gains_operateur AS
SELECT op.nomOperateur AS nom_operateur, COALESCE(SUM(t.frais), 0) AS total_frais
FROM
    operateurs op
    ...
    LEFT JOIN types_operations toper ON toper.id = t.type_operation_id
    AND toper.libelle IN ('Retrait', 'Transfert')
GROUP BY
    op.nomOperateur;
...
FROM
    transactions t
    JOIN types_operations toper ON toper.id = t.type_operation_id
    LEFT JOIN comptes src ON src.id = t.compte_source_id
    LEFT JOIN comptes dest ON dest.id = t.compte_destination_id;